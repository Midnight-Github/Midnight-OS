---@diagnostic disable: undefined-field, ambiguity-1, cast-local-type, duplicate-set-field, undefined-global, redundant-parameter
...
self.set("offset",ca)self:fireEvent("mouse_scroll",_a,aa,ba)return true end;return false end
function d:selectItem(_a)local aa=self.get("items")if
not self.get("multiSelection")then
for ca,da in ipairs(aa)do if type(da)=="table"then da.selected=false end end end;local ba=aa[_a]if
type(ba)=="string"then ba={text=ba}aa[_a]=ba end
ba.selected=true;if ba.callback then ba.callback(self)end
self:fireEvent("select",_a,ba)self:updateRender()return self end
...
self:textFg(db,1,bc:sub(1,ac.width),self.get("headerColor"))db=db+ac.width end
for y=2,bb do local _c=y-2 +ab;local ac=ca[_c+1]
if ac and(_c+1)<=#ca then db=1
local bc=(_c+1)==da and
self.get("selectedColor")or self.get("background")
for cc,dc in ipairs(ba)do local _d=tostring(ac[cc]or"")local ad=_d..
string.rep(" ",dc.width-#_d)if cc<#ba then ad=
string.sub(ad,1,dc.width-1).." "end
